import AllIcon from './AllIcon'
import AndroidIcon from './AndroidIcon'
import ApiIcon from './ApiIcon'
import AppCheckIcon from './AppCheckIcon'
import AppPlusIcon from './AppPlusIcon'
import AppPushIcon from './AppPushIcon'
import AppleIcon from './AppleIcon'
import AppsAddIcon from './AppsAddIcon'
import AppsFilledIcon from './AppsFilledIcon'
import AppsIcon from './AppsIcon'
import ArchiveIcon from './ArchiveIcon'
import ArrowDownIcon from './ArrowDownIcon'
import ArrowHookLeftDownIcon from './ArrowHookLeftDownIcon'
import ArrowHookLeftUpIcon from './ArrowHookLeftUpIcon'
import ArrowHookRightDownIcon from './ArrowHookRightDownIcon'
import ArrowHookRightUpIcon from './ArrowHookRightUpIcon'
import ArrowLeftCircleFilledIcon from './ArrowLeftCircleFilledIcon'
import ArrowLeftIcon from './ArrowLeftIcon'
import ArrowLeftUpIcon from './ArrowLeftUpIcon'
import ArrowRightIcon from './ArrowRightIcon'
import ArrowRightUpIcon from './ArrowRightUpIcon'
import ArrowRightUpSmallIcon from './ArrowRightUpSmallIcon'
import ArrowTurnLeftDownIcon from './ArrowTurnLeftDownIcon'
import ArrowTurnLeftUpIcon from './ArrowTurnLeftUpIcon'
import ArrowTurnRightDownIcon from './ArrowTurnRightDownIcon'
import ArrowTurnRightUpIcon from './ArrowTurnRightUpIcon'
import ArrowUpIcon from './ArrowUpIcon'
import AssigneeIcon from './AssigneeIcon'
import AtomIcon from './AtomIcon'
import BadgeIcon from './BadgeIcon'
import BankIcon from './BankIcon'
import BaseballIcon from './BaseballIcon'
import BasketballIcon from './BasketballIcon'
import BearFilledIcon from './BearFilledIcon'
import BearIcon from './BearIcon'
import BlockIcon from './BlockIcon'
import BoldIcon from './BoldIcon'
import BookCoverIcon from './BookCoverIcon'
import BookIcon from './BookIcon'
import BookmarkFilledIcon from './BookmarkFilledIcon'
import BookmarkIcon from './BookmarkIcon'
import BooleanIcon from './BooleanIcon'
import BotFilledIcon from './BotFilledIcon'
import BotIcon from './BotIcon'
import BreaktimeIcon from './BreaktimeIcon'
import BriefcaseIcon from './BriefcaseIcon'
import BrowserChromeIcon from './BrowserChromeIcon'
import BrowserEdgeIcon from './BrowserEdgeIcon'
import BrowserFirefoxIcon from './BrowserFirefoxIcon'
import BrowserIeIcon from './BrowserIeIcon'
import BrowserSafariIcon from './BrowserSafariIcon'
import BulletIcon from './BulletIcon'
import BusIcon from './BusIcon'
import BusinessGuyIcon from './BusinessGuyIcon'
import ButtonRouteIcon from './ButtonRouteIcon'
import CalendarIcon from './CalendarIcon'
import CallIcon from './CallIcon'
import CallMissedIcon from './CallMissedIcon'
import CallOffIcon from './CallOffIcon'
import CallOutgoingIcon from './CallOutgoingIcon'
import CallPullIcon from './CallPullIcon'
import CameraIcon from './CameraIcon'
import CancelCircleFilledIcon from './CancelCircleFilledIcon'
import CancelCircleIcon from './CancelCircleIcon'
import CancelIcon from './CancelIcon'
import CancelSmallIcon from './CancelSmallIcon'
import CarIcon from './CarIcon'
import CartAbandonedIcon from './CartAbandonedIcon'
import CartIcon from './CartIcon'
import ChainReactionIcon from './ChainReactionIcon'
import ChannelBtnFilledIcon from './ChannelBtnFilledIcon'
import ChannelBtnIcon from './ChannelBtnIcon'
import ChannelBtnSmileFilledIcon from './ChannelBtnSmileFilledIcon'
import ChannelBtnSmileIcon from './ChannelBtnSmileIcon'
import ChannelFilledIcon from './ChannelFilledIcon'
import ChannelIcon from './ChannelIcon'
import ChannelLeftFilledIcon from './ChannelLeftFilledIcon'
import ChannelLeftIcon from './ChannelLeftIcon'
import ChannelSmileFilledIcon from './ChannelSmileFilledIcon'
import ChannelSmileIcon from './ChannelSmileIcon'
import ChannelUpFilledIcon from './ChannelUpFilledIcon'
import ChannelUpIcon from './ChannelUpIcon'
import ChannelsListIcon from './ChannelsListIcon'
import ChatBubbleFilledIcon from './ChatBubbleFilledIcon'
import ChatBubbleIcon from './ChatBubbleIcon'
import ChatCancelIcon from './ChatCancelIcon'
import ChatCheckFilledIcon from './ChatCheckFilledIcon'
import ChatCheckIcon from './ChatCheckIcon'
import ChatErrorFilledIcon from './ChatErrorFilledIcon'
import ChatErrorIcon from './ChatErrorIcon'
import ChatForwardIcon from './ChatForwardIcon'
import ChatInfoIcon from './ChatInfoIcon'
import ChatLightningIcon from './ChatLightningIcon'
import ChatProgressFilledIcon from './ChatProgressFilledIcon'
import ChatProgressIcon from './ChatProgressIcon'
import ChatPushIcon from './ChatPushIcon'
import ChatQuestionFilledIcon from './ChatQuestionFilledIcon'
import ChatQuestionIcon from './ChatQuestionIcon'
import ChatReadyIcon from './ChatReadyIcon'
import CheckAllIcon from './CheckAllIcon'
import CheckBoldIcon from './CheckBoldIcon'
import CheckCircleFilledIcon from './CheckCircleFilledIcon'
import CheckCircleIcon from './CheckCircleIcon'
import CheckIcon from './CheckIcon'
import CheckboxFilledIcon from './CheckboxFilledIcon'
import CheckboxIcon from './CheckboxIcon'
import ChevronDownDoubleIcon from './ChevronDownDoubleIcon'
import ChevronDownIcon from './ChevronDownIcon'
import ChevronLeftDoubleIcon from './ChevronLeftDoubleIcon'
import ChevronLeftIcon from './ChevronLeftIcon'
import ChevronRightDoubleIcon from './ChevronRightDoubleIcon'
import ChevronRightIcon from './ChevronRightIcon'
import ChevronSmallDownIcon from './ChevronSmallDownIcon'
import ChevronSmallLeftIcon from './ChevronSmallLeftIcon'
import ChevronSmallRightIcon from './ChevronSmallRightIcon'
import ChevronSmallUpIcon from './ChevronSmallUpIcon'
import ChevronSmallUpdownIcon from './ChevronSmallUpdownIcon'
import ChevronUpDoubleIcon from './ChevronUpDoubleIcon'
import ChevronUpIcon from './ChevronUpIcon'
import ChevronUpdownIcon from './ChevronUpdownIcon'
import ClipIcon from './ClipIcon'
import ClockIcon from './ClockIcon'
import CloudDownloadIcon from './CloudDownloadIcon'
import CloudUploadIcon from './CloudUploadIcon'
import CodeIcon from './CodeIcon'
import CoinIcon from './CoinIcon'
import CoinRecurringIcon from './CoinRecurringIcon'
import ColumnIcon from './ColumnIcon'
import CommentFilledIcon from './CommentFilledIcon'
import CommentIcon from './CommentIcon'
import CommentInIcon from './CommentInIcon'
import CommentOutIcon from './CommentOutIcon'
import CommunicationIcon from './CommunicationIcon'
import ContactIcon from './ContactIcon'
import ContractIcon from './ContractIcon'
import CookieIcon from './CookieIcon'
import CouponIcon from './CouponIcon'
import CreditIcon from './CreditIcon'
import CreditcardIcon from './CreditcardIcon'
import DataIcon from './DataIcon'
import DesktopIcon from './DesktopIcon'
import DevicesIcon from './DevicesIcon'
import DialogDownIcon from './DialogDownIcon'
import DialogUpIcon from './DialogUpIcon'
import DotIcon from './DotIcon'
import DownloadIcon from './DownloadIcon'
import DragableIcon from './DragableIcon'
import DragableSmallIcon from './DragableSmallIcon'
import DropdownIcon from './DropdownIcon'
import EditIcon from './EditIcon'
import EmailFilledIcon from './EmailFilledIcon'
import EmailIcon from './EmailIcon'
import EmailUnreadIcon from './EmailUnreadIcon'
import EmailUnsubscribedIcon from './EmailUnsubscribedIcon'
import ErrorFilledIcon from './ErrorFilledIcon'
import ErrorIcon from './ErrorIcon'
import ErrorTriangleFilledIcon from './ErrorTriangleFilledIcon'
import EventIcon from './EventIcon'
import ExpandIcon from './ExpandIcon'
import FaceAngryIcon from './FaceAngryIcon'
import FaceGrinningIcon from './FaceGrinningIcon'
import FaceNomouthIcon from './FaceNomouthIcon'
import FaceSadIcon from './FaceSadIcon'
import FaceSmileAddIcon from './FaceSmileAddIcon'
import FaceSmileFilledIcon from './FaceSmileFilledIcon'
import FaceSmileIcon from './FaceSmileIcon'
import FaceWowIcon from './FaceWowIcon'
import FacebookIcon from './FacebookIcon'
import FastForwardIcon from './FastForwardIcon'
import FastRewindIcon from './FastRewindIcon'
import FireIcon from './FireIcon'
import FlagFilledIcon from './FlagFilledIcon'
import FlashlightIcon from './FlashlightIcon'
import FlushIcon from './FlushIcon'
import FolderIcon from './FolderIcon'
import FullscreenExitIcon from './FullscreenExitIcon'
import FullscreenIcon from './FullscreenIcon'
import FunnelIcon from './FunnelIcon'
import GhostFilledIcon from './GhostFilledIcon'
import GhostIcon from './GhostIcon'
import GifIcon from './GifIcon'
import GiftIcon from './GiftIcon'
import GlobeIcon from './GlobeIcon'
import GoalIcon from './GoalIcon'
import GoogleIcon from './GoogleIcon'
import GraphIcon from './GraphIcon'
import GroupFilledIcon from './GroupFilledIcon'
import GroupIcon from './GroupIcon'
import HandIcon from './HandIcon'
import HandLoveIcon from './HandLoveIcon'
import HandPointIcon from './HandPointIcon'
import HandPushIcon from './HandPushIcon'
import HandThumbdownFilledIcon from './HandThumbdownFilledIcon'
import HandThumbdownIcon from './HandThumbdownIcon'
import HandThumbupFilledIcon from './HandThumbupFilledIcon'
import HandThumbupIcon from './HandThumbupIcon'
import HandWaveIcon from './HandWaveIcon'
import HashtagIcon from './HashtagIcon'
import HashtagLargeIcon from './HashtagLargeIcon'
import HeadingIcon from './HeadingIcon'
import HeadsetIcon from './HeadsetIcon'
import HeadsetOffIcon from './HeadsetOffIcon'
import HearIcon from './HearIcon'
import HeartFilledIcon from './HeartFilledIcon'
import HeartIcon from './HeartIcon'
import HelpFilledIcon from './HelpFilledIcon'
import HelpIcon from './HelpIcon'
import HexahedronIcon from './HexahedronIcon'
import HistoryIcon from './HistoryIcon'
import HomeFilledIcon from './HomeFilledIcon'
import HomeIcon from './HomeIcon'
import Hourglass1Icon from './Hourglass1Icon'
import Hourglass2Icon from './Hourglass2Icon'
import Hourglass3Icon from './Hourglass3Icon'
import HyphenBoldIcon from './HyphenBoldIcon'
import HyphenIcon from './HyphenIcon'
import ImageFailedIcon from './ImageFailedIcon'
import ImageIcon from './ImageIcon'
import InIcon from './InIcon'
import InboxAllIcon from './InboxAllIcon'
import InboxIcon from './InboxIcon'
import InfoFilledIcon from './InfoFilledIcon'
import InfoIcon from './InfoIcon'
import InstagramIcon from './InstagramIcon'
import IntoPageIcon from './IntoPageIcon'
import InvertIcon from './InvertIcon'
import IosIcon from './IosIcon'
import ItalicIcon from './ItalicIcon'
import KakaoFilledIcon from './KakaoFilledIcon'
import KakaoIcon from './KakaoIcon'
import KeyIcon from './KeyIcon'
import KeyboardIcon from './KeyboardIcon'
import LabIcon from './LabIcon'
import LaptopIcon from './LaptopIcon'
import LightbulbIcon from './LightbulbIcon'
import LightningFilledIcon from './LightningFilledIcon'
import LightningIcon from './LightningIcon'
import LinkCopyIcon from './LinkCopyIcon'
import LinkIcon from './LinkIcon'
import LinkOffIcon from './LinkOffIcon'
import LinkedinIcon from './LinkedinIcon'
import ListIcon from './ListIcon'
import LockIcon from './LockIcon'
import LockOpenIcon from './LockOpenIcon'
import MapPinIcon from './MapPinIcon'
import MarkerPenIcon from './MarkerPenIcon'
import MeIcon from './MeIcon'
import MegaphoneFilledIcon from './MegaphoneFilledIcon'
import MegaphoneIcon from './MegaphoneIcon'
import MenuFoldIcon from './MenuFoldIcon'
import MenuIcon from './MenuIcon'
import MenuUnfoldIcon from './MenuUnfoldIcon'
import MessagingAppIcon from './MessagingAppIcon'
import MetroIcon from './MetroIcon'
import MicrophoneIcon from './MicrophoneIcon'
import MicrophoneOffIcon from './MicrophoneOffIcon'
import MinusCircleFilledIcon from './MinusCircleFilledIcon'
import MinusCircleIcon from './MinusCircleIcon'
import MobileIcon from './MobileIcon'
import MobileMessagingIcon from './MobileMessagingIcon'
import MoonFilledIcon from './MoonFilledIcon'
import MoonIcon from './MoonIcon'
import MoreIcon from './MoreIcon'
import MoreVerticalIcon from './MoreVerticalIcon'
import MouseIcon from './MouseIcon'
import MultiNodeIcon from './MultiNodeIcon'
import MusicIcon from './MusicIcon'
import NaverIcon from './NaverIcon'
import NewIcon from './NewIcon'
import NoteIcon from './NoteIcon'
import NoteLogoIcon from './NoteLogoIcon'
import NotificationIcon from './NotificationIcon'
import NotificationImportantIcon from './NotificationImportantIcon'
import NotificationOffIcon from './NotificationOffIcon'
import NumberIcon from './NumberIcon'
import OfficeIcon from './OfficeIcon'
import OpenInNewIcon from './OpenInNewIcon'
import OrderIcon from './OrderIcon'
import OutIcon from './OutIcon'
import PageDownloadIcon from './PageDownloadIcon'
import PageIcon from './PageIcon'
import PageZipIcon from './PageZipIcon'
import PauseIcon from './PauseIcon'
import PencilIcon from './PencilIcon'
import PeopleIcon from './PeopleIcon'
import PeopleListIcon from './PeopleListIcon'
import PersonAddIcon from './PersonAddIcon'
import PersonBlockedIcon from './PersonBlockedIcon'
import PersonCheckIcon from './PersonCheckIcon'
import PersonCircleFilledIcon from './PersonCircleFilledIcon'
import PersonCircleIcon from './PersonCircleIcon'
import PersonFilledIcon from './PersonFilledIcon'
import PersonIcon from './PersonIcon'
import PersonRemoveIcon from './PersonRemoveIcon'
import PieChartIcon from './PieChartIcon'
import PinIcon from './PinIcon'
import PinOffIcon from './PinOffIcon'
import PlaneIcon from './PlaneIcon'
import PlayIcon from './PlayIcon'
import PlusCircleFilledIcon from './PlusCircleFilledIcon'
import PlusCircleIcon from './PlusCircleIcon'
import PlusIcon from './PlusIcon'
import PlusSmallIcon from './PlusSmallIcon'
import PointIcon from './PointIcon'
import PowerIcon from './PowerIcon'
import PrintIcon from './PrintIcon'
import ProfileIcon from './ProfileIcon'
import QrCodeIcon from './QrCodeIcon'
import RadioFilledIcon from './RadioFilledIcon'
import RealtimeIcon from './RealtimeIcon'
import ReceiptIcon from './ReceiptIcon'
import RecipeIcon from './RecipeIcon'
import RefreshCircleFilledIcon from './RefreshCircleFilledIcon'
import RefreshIcon from './RefreshIcon'
import RepeatIcon from './RepeatIcon'
import RotateIcon from './RotateIcon'
import RowIcon from './RowIcon'
import RulerIcon from './RulerIcon'
import SearchIcon from './SearchIcon'
import SecurityIcon from './SecurityIcon'
import SecurityPersonIcon from './SecurityPersonIcon'
import SendFilledIcon from './SendFilledIcon'
import SendForwardFailedFilledIcon from './SendForwardFailedFilledIcon'
import SendForwardFailedIcon from './SendForwardFailedIcon'
import SendForwardFilledIcon from './SendForwardFilledIcon'
import SendForwardIcon from './SendForwardIcon'
import SendIcon from './SendIcon'
import SettingsIcon from './SettingsIcon'
import ShapeIcon from './ShapeIcon'
import ShineIcon from './ShineIcon'
import ShoppingIcon from './ShoppingIcon'
import ShuffleIcon from './ShuffleIcon'
import SingleNodeIcon from './SingleNodeIcon'
import SirenIcon from './SirenIcon'
import SmsFilledIcon from './SmsFilledIcon'
import SmsIcon from './SmsIcon'
import SmsUnsubscribedIcon from './SmsUnsubscribedIcon'
import SnoozeIcon from './SnoozeIcon'
import SoccerballIcon from './SoccerballIcon'
import SortingIcon from './SortingIcon'
import SpaceHorizontalIcon from './SpaceHorizontalIcon'
import SpaceVerticalIcon from './SpaceVerticalIcon'
import SplitLeftIcon from './SplitLeftIcon'
import SplitRightIcon from './SplitRightIcon'
import SquareIcon from './SquareIcon'
import SquaresFilledIcon from './SquaresFilledIcon'
import SquaresIcon from './SquaresIcon'
import StarFilledIcon from './StarFilledIcon'
import StarIcon from './StarIcon'
import StoreIcon from './StoreIcon'
import StrikethroughIcon from './StrikethroughIcon'
import StringIcon from './StringIcon'
import SurveyCheckIcon from './SurveyCheckIcon'
import SurveyIcon from './SurveyIcon'
import SyncIcon from './SyncIcon'
import TabletIcon from './TabletIcon'
import TagIcon from './TagIcon'
import TargetIcon from './TargetIcon'
import TemplateIcon from './TemplateIcon'
import TextfieldIcon from './TextfieldIcon'
import TimeElapsedIcon from './TimeElapsedIcon'
import ToolIcon from './ToolIcon'
import TransferDisabledIcon from './TransferDisabledIcon'
import TransferIcon from './TransferIcon'
import TranslateIcon from './TranslateIcon'
import TrashIcon from './TrashIcon'
import TrendingDownIcon from './TrendingDownIcon'
import TrendingLeftCircleFilledIcon from './TrendingLeftCircleFilledIcon'
import TrendingLeftIcon from './TrendingLeftIcon'
import TrendingRightIcon from './TrendingRightIcon'
import TrendingUpIcon from './TrendingUpIcon'
import TriangleDownCircleFilledIcon from './TriangleDownCircleFilledIcon'
import TriangleDownCircleIcon from './TriangleDownCircleIcon'
import TriangleDownIcon from './TriangleDownIcon'
import TriangleLeftCircleFilledIcon from './TriangleLeftCircleFilledIcon'
import TriangleLeftCircleIcon from './TriangleLeftCircleIcon'
import TriangleLeftIcon from './TriangleLeftIcon'
import TriangleRightCircleFilledIcon from './TriangleRightCircleFilledIcon'
import TriangleRightCircleIcon from './TriangleRightCircleIcon'
import TriangleRightIcon from './TriangleRightIcon'
import TriangleUpCircleFilledIcon from './TriangleUpCircleFilledIcon'
import TriangleUpCircleIcon from './TriangleUpCircleIcon'
import TriangleUpIcon from './TriangleUpIcon'
import TriangleUpdownIcon from './TriangleUpdownIcon'
import TrophyIcon from './TrophyIcon'
import TuneIcon from './TuneIcon'
import TwitterIcon from './TwitterIcon'
import TypographyIcon from './TypographyIcon'
import UnderlineIcon from './UnderlineIcon'
import UntagIcon from './UntagIcon'
import UploadIcon from './UploadIcon'
import UsernameIcon from './UsernameIcon'
import VariableIcon from './VariableIcon'
import VideocamIcon from './VideocamIcon'
import VideocamOffIcon from './VideocamOffIcon'
import ViewIcon from './ViewIcon'
import ViewOffIcon from './ViewOffIcon'
import VolumeDownIcon from './VolumeDownIcon'
import VolumeOffFilledIcon from './VolumeOffFilledIcon'
import VolumeOffIcon from './VolumeOffIcon'
import VolumeUpFilledIcon from './VolumeUpFilledIcon'
import VolumeUpIcon from './VolumeUpIcon'
import WalletIcon from './WalletIcon'
import WaterIcon from './WaterIcon'
import WeatherCloudIcon from './WeatherCloudIcon'
import WeatherCloudyIcon from './WeatherCloudyIcon'
import WeatherRainIcon from './WeatherRainIcon'
import WeatherSnowIcon from './WeatherSnowIcon'
import WeatherSunIcon from './WeatherSunIcon'
import WeatherThunderIcon from './WeatherThunderIcon'
import WebhookIcon from './WebhookIcon'
import WifiIcon from './WifiIcon'
import WifiOffIcon from './WifiOffIcon'
import WindowsCloseIcon from './WindowsCloseIcon'
import WindowsIcon from './WindowsIcon'
import WindowsMaximizeIcon from './WindowsMaximizeIcon'
import WindowsMinimizeIcon from './WindowsMinimizeIcon'
import WindowsRestoreIcon from './WindowsRestoreIcon'
import WingIcon from './WingIcon'
import YoutubeIcon from './YoutubeIcon'
import ZoomInIcon from './ZoomInIcon'
import ZoomOutIcon from './ZoomOutIcon'
/* eslint-disable */

const icons = {
  all: AllIcon,
  android: AndroidIcon,
  api: ApiIcon,
  'app-check': AppCheckIcon,
  'app-plus': AppPlusIcon,
  'app-push': AppPushIcon,
  apple: AppleIcon,
  'apps-add': AppsAddIcon,
  'apps-filled': AppsFilledIcon,
  apps: AppsIcon,
  archive: ArchiveIcon,
  'arrow-down': ArrowDownIcon,
  'arrow-hook-left-down': ArrowHookLeftDownIcon,
  'arrow-hook-left-up': ArrowHookLeftUpIcon,
  'arrow-hook-right-down': ArrowHookRightDownIcon,
  'arrow-hook-right-up': ArrowHookRightUpIcon,
  'arrow-left-circle-filled': ArrowLeftCircleFilledIcon,
  'arrow-left': ArrowLeftIcon,
  'arrow-left-up': ArrowLeftUpIcon,
  'arrow-right': ArrowRightIcon,
  'arrow-right-up': ArrowRightUpIcon,
  'arrow-right-up-small': ArrowRightUpSmallIcon,
  'arrow-turn-left-down': ArrowTurnLeftDownIcon,
  'arrow-turn-left-up': ArrowTurnLeftUpIcon,
  'arrow-turn-right-down': ArrowTurnRightDownIcon,
  'arrow-turn-right-up': ArrowTurnRightUpIcon,
  'arrow-up': ArrowUpIcon,
  assignee: AssigneeIcon,
  atom: AtomIcon,
  badge: BadgeIcon,
  bank: BankIcon,
  baseball: BaseballIcon,
  basketball: BasketballIcon,
  'bear-filled': BearFilledIcon,
  bear: BearIcon,
  block: BlockIcon,
  bold: BoldIcon,
  'book-cover': BookCoverIcon,
  book: BookIcon,
  'bookmark-filled': BookmarkFilledIcon,
  bookmark: BookmarkIcon,
  boolean: BooleanIcon,
  'bot-filled': BotFilledIcon,
  bot: BotIcon,
  breaktime: BreaktimeIcon,
  briefcase: BriefcaseIcon,
  'browser-chrome': BrowserChromeIcon,
  'browser-edge': BrowserEdgeIcon,
  'browser-firefox': BrowserFirefoxIcon,
  'browser-ie': BrowserIeIcon,
  'browser-safari': BrowserSafariIcon,
  bullet: BulletIcon,
  bus: BusIcon,
  'business-guy': BusinessGuyIcon,
  'button-route': ButtonRouteIcon,
  calendar: CalendarIcon,
  call: CallIcon,
  'call-missed': CallMissedIcon,
  'call-off': CallOffIcon,
  'call-outgoing': CallOutgoingIcon,
  'call-pull': CallPullIcon,
  camera: CameraIcon,
  'cancel-circle-filled': CancelCircleFilledIcon,
  'cancel-circle': CancelCircleIcon,
  cancel: CancelIcon,
  'cancel-small': CancelSmallIcon,
  car: CarIcon,
  'cart-abandoned': CartAbandonedIcon,
  cart: CartIcon,
  'chain-reaction': ChainReactionIcon,
  'channel-btn-filled': ChannelBtnFilledIcon,
  'channel-btn': ChannelBtnIcon,
  'channel-btn-smile-filled': ChannelBtnSmileFilledIcon,
  'channel-btn-smile': ChannelBtnSmileIcon,
  'channel-filled': ChannelFilledIcon,
  channel: ChannelIcon,
  'channel-left-filled': ChannelLeftFilledIcon,
  'channel-left': ChannelLeftIcon,
  'channel-smile-filled': ChannelSmileFilledIcon,
  'channel-smile': ChannelSmileIcon,
  'channel-up-filled': ChannelUpFilledIcon,
  'channel-up': ChannelUpIcon,
  'channels-list': ChannelsListIcon,
  'chat-bubble-filled': ChatBubbleFilledIcon,
  'chat-bubble': ChatBubbleIcon,
  'chat-cancel': ChatCancelIcon,
  'chat-check-filled': ChatCheckFilledIcon,
  'chat-check': ChatCheckIcon,
  'chat-error-filled': ChatErrorFilledIcon,
  'chat-error': ChatErrorIcon,
  'chat-forward': ChatForwardIcon,
  'chat-info': ChatInfoIcon,
  'chat-lightning': ChatLightningIcon,
  'chat-progress-filled': ChatProgressFilledIcon,
  'chat-progress': ChatProgressIcon,
  'chat-push': ChatPushIcon,
  'chat-question-filled': ChatQuestionFilledIcon,
  'chat-question': ChatQuestionIcon,
  'chat-ready': ChatReadyIcon,
  'check-all': CheckAllIcon,
  'check-bold': CheckBoldIcon,
  'check-circle-filled': CheckCircleFilledIcon,
  'check-circle': CheckCircleIcon,
  check: CheckIcon,
  'checkbox-filled': CheckboxFilledIcon,
  checkbox: CheckboxIcon,
  'chevron-down-double': ChevronDownDoubleIcon,
  'chevron-down': ChevronDownIcon,
  'chevron-left-double': ChevronLeftDoubleIcon,
  'chevron-left': ChevronLeftIcon,
  'chevron-right-double': ChevronRightDoubleIcon,
  'chevron-right': ChevronRightIcon,
  'chevron-small-down': ChevronSmallDownIcon,
  'chevron-small-left': ChevronSmallLeftIcon,
  'chevron-small-right': ChevronSmallRightIcon,
  'chevron-small-up': ChevronSmallUpIcon,
  'chevron-small-updown': ChevronSmallUpdownIcon,
  'chevron-up-double': ChevronUpDoubleIcon,
  'chevron-up': ChevronUpIcon,
  'chevron-updown': ChevronUpdownIcon,
  clip: ClipIcon,
  clock: ClockIcon,
  'cloud-download': CloudDownloadIcon,
  'cloud-upload': CloudUploadIcon,
  code: CodeIcon,
  coin: CoinIcon,
  'coin-recurring': CoinRecurringIcon,
  column: ColumnIcon,
  'comment-filled': CommentFilledIcon,
  comment: CommentIcon,
  'comment-in': CommentInIcon,
  'comment-out': CommentOutIcon,
  communication: CommunicationIcon,
  contact: ContactIcon,
  contract: ContractIcon,
  cookie: CookieIcon,
  coupon: CouponIcon,
  credit: CreditIcon,
  creditcard: CreditcardIcon,
  data: DataIcon,
  desktop: DesktopIcon,
  devices: DevicesIcon,
  'dialog-down': DialogDownIcon,
  'dialog-up': DialogUpIcon,
  dot: DotIcon,
  download: DownloadIcon,
  dragable: DragableIcon,
  'dragable-small': DragableSmallIcon,
  dropdown: DropdownIcon,
  edit: EditIcon,
  'email-filled': EmailFilledIcon,
  email: EmailIcon,
  'email-unread': EmailUnreadIcon,
  'email-unsubscribed': EmailUnsubscribedIcon,
  'error-filled': ErrorFilledIcon,
  error: ErrorIcon,
  'error-triangle-filled': ErrorTriangleFilledIcon,
  event: EventIcon,
  expand: ExpandIcon,
  'face-angry': FaceAngryIcon,
  'face-grinning': FaceGrinningIcon,
  'face-nomouth': FaceNomouthIcon,
  'face-sad': FaceSadIcon,
  'face-smile-add': FaceSmileAddIcon,
  'face-smile-filled': FaceSmileFilledIcon,
  'face-smile': FaceSmileIcon,
  'face-wow': FaceWowIcon,
  facebook: FacebookIcon,
  'fast-forward': FastForwardIcon,
  'fast-rewind': FastRewindIcon,
  fire: FireIcon,
  'flag-filled': FlagFilledIcon,
  flashlight: FlashlightIcon,
  flush: FlushIcon,
  folder: FolderIcon,
  'fullscreen-exit': FullscreenExitIcon,
  fullscreen: FullscreenIcon,
  funnel: FunnelIcon,
  'ghost-filled': GhostFilledIcon,
  ghost: GhostIcon,
  gif: GifIcon,
  gift: GiftIcon,
  globe: GlobeIcon,
  goal: GoalIcon,
  google: GoogleIcon,
  graph: GraphIcon,
  'group-filled': GroupFilledIcon,
  group: GroupIcon,
  hand: HandIcon,
  'hand-love': HandLoveIcon,
  'hand-point': HandPointIcon,
  'hand-push': HandPushIcon,
  'hand-thumbdown-filled': HandThumbdownFilledIcon,
  'hand-thumbdown': HandThumbdownIcon,
  'hand-thumbup-filled': HandThumbupFilledIcon,
  'hand-thumbup': HandThumbupIcon,
  'hand-wave': HandWaveIcon,
  hashtag: HashtagIcon,
  'hashtag-large': HashtagLargeIcon,
  heading: HeadingIcon,
  headset: HeadsetIcon,
  'headset-off': HeadsetOffIcon,
  hear: HearIcon,
  'heart-filled': HeartFilledIcon,
  heart: HeartIcon,
  'help-filled': HelpFilledIcon,
  help: HelpIcon,
  hexahedron: HexahedronIcon,
  history: HistoryIcon,
  'home-filled': HomeFilledIcon,
  home: HomeIcon,
  'hourglass-1': Hourglass1Icon,
  'hourglass-2': Hourglass2Icon,
  'hourglass-3': Hourglass3Icon,
  'hyphen-bold': HyphenBoldIcon,
  hyphen: HyphenIcon,
  'image-failed': ImageFailedIcon,
  image: ImageIcon,
  in: InIcon,
  'inbox-all': InboxAllIcon,
  inbox: InboxIcon,
  'info-filled': InfoFilledIcon,
  info: InfoIcon,
  instagram: InstagramIcon,
  'into-page': IntoPageIcon,
  invert: InvertIcon,
  ios: IosIcon,
  italic: ItalicIcon,
  'kakao-filled': KakaoFilledIcon,
  kakao: KakaoIcon,
  key: KeyIcon,
  keyboard: KeyboardIcon,
  lab: LabIcon,
  laptop: LaptopIcon,
  lightbulb: LightbulbIcon,
  'lightning-filled': LightningFilledIcon,
  lightning: LightningIcon,
  'link-copy': LinkCopyIcon,
  link: LinkIcon,
  'link-off': LinkOffIcon,
  linkedin: LinkedinIcon,
  list: ListIcon,
  lock: LockIcon,
  'lock-open': LockOpenIcon,
  'map-pin': MapPinIcon,
  'marker-pen': MarkerPenIcon,
  me: MeIcon,
  'megaphone-filled': MegaphoneFilledIcon,
  megaphone: MegaphoneIcon,
  'menu-fold': MenuFoldIcon,
  menu: MenuIcon,
  'menu-unfold': MenuUnfoldIcon,
  'messaging-app': MessagingAppIcon,
  metro: MetroIcon,
  microphone: MicrophoneIcon,
  'microphone-off': MicrophoneOffIcon,
  'minus-circle-filled': MinusCircleFilledIcon,
  'minus-circle': MinusCircleIcon,
  mobile: MobileIcon,
  'mobile-messaging': MobileMessagingIcon,
  'moon-filled': MoonFilledIcon,
  moon: MoonIcon,
  more: MoreIcon,
  'more-vertical': MoreVerticalIcon,
  mouse: MouseIcon,
  'multi-node': MultiNodeIcon,
  music: MusicIcon,
  naver: NaverIcon,
  new: NewIcon,
  note: NoteIcon,
  'note-logo': NoteLogoIcon,
  notification: NotificationIcon,
  'notification-important': NotificationImportantIcon,
  'notification-off': NotificationOffIcon,
  number: NumberIcon,
  office: OfficeIcon,
  'open-in-new': OpenInNewIcon,
  order: OrderIcon,
  out: OutIcon,
  'page-download': PageDownloadIcon,
  page: PageIcon,
  'page-zip': PageZipIcon,
  pause: PauseIcon,
  pencil: PencilIcon,
  people: PeopleIcon,
  'people-list': PeopleListIcon,
  'person-add': PersonAddIcon,
  'person-blocked': PersonBlockedIcon,
  'person-check': PersonCheckIcon,
  'person-circle-filled': PersonCircleFilledIcon,
  'person-circle': PersonCircleIcon,
  'person-filled': PersonFilledIcon,
  person: PersonIcon,
  'person-remove': PersonRemoveIcon,
  'pie-chart': PieChartIcon,
  pin: PinIcon,
  'pin-off': PinOffIcon,
  plane: PlaneIcon,
  play: PlayIcon,
  'plus-circle-filled': PlusCircleFilledIcon,
  'plus-circle': PlusCircleIcon,
  plus: PlusIcon,
  'plus-small': PlusSmallIcon,
  point: PointIcon,
  power: PowerIcon,
  print: PrintIcon,
  profile: ProfileIcon,
  'qr-code': QrCodeIcon,
  'radio-filled': RadioFilledIcon,
  realtime: RealtimeIcon,
  receipt: ReceiptIcon,
  recipe: RecipeIcon,
  'refresh-circle-filled': RefreshCircleFilledIcon,
  refresh: RefreshIcon,
  repeat: RepeatIcon,
  rotate: RotateIcon,
  row: RowIcon,
  ruler: RulerIcon,
  search: SearchIcon,
  security: SecurityIcon,
  'security-person': SecurityPersonIcon,
  'send-filled': SendFilledIcon,
  'send-forward-failed-filled': SendForwardFailedFilledIcon,
  'send-forward-failed': SendForwardFailedIcon,
  'send-forward-filled': SendForwardFilledIcon,
  'send-forward': SendForwardIcon,
  send: SendIcon,
  settings: SettingsIcon,
  shape: ShapeIcon,
  shine: ShineIcon,
  shopping: ShoppingIcon,
  shuffle: ShuffleIcon,
  'single-node': SingleNodeIcon,
  siren: SirenIcon,
  'sms-filled': SmsFilledIcon,
  sms: SmsIcon,
  'sms-unsubscribed': SmsUnsubscribedIcon,
  snooze: SnoozeIcon,
  soccerball: SoccerballIcon,
  sorting: SortingIcon,
  'space-horizontal': SpaceHorizontalIcon,
  'space-vertical': SpaceVerticalIcon,
  'split-left': SplitLeftIcon,
  'split-right': SplitRightIcon,
  square: SquareIcon,
  'squares-filled': SquaresFilledIcon,
  squares: SquaresIcon,
  'star-filled': StarFilledIcon,
  star: StarIcon,
  store: StoreIcon,
  strikethrough: StrikethroughIcon,
  string: StringIcon,
  'survey-check': SurveyCheckIcon,
  survey: SurveyIcon,
  sync: SyncIcon,
  tablet: TabletIcon,
  tag: TagIcon,
  target: TargetIcon,
  template: TemplateIcon,
  textfield: TextfieldIcon,
  'time-elapsed': TimeElapsedIcon,
  tool: ToolIcon,
  'transfer-disabled': TransferDisabledIcon,
  transfer: TransferIcon,
  translate: TranslateIcon,
  trash: TrashIcon,
  'trending-down': TrendingDownIcon,
  'trending-left-circle-filled': TrendingLeftCircleFilledIcon,
  'trending-left': TrendingLeftIcon,
  'trending-right': TrendingRightIcon,
  'trending-up': TrendingUpIcon,
  'triangle-down-circle-filled': TriangleDownCircleFilledIcon,
  'triangle-down-circle': TriangleDownCircleIcon,
  'triangle-down': TriangleDownIcon,
  'triangle-left-circle-filled': TriangleLeftCircleFilledIcon,
  'triangle-left-circle': TriangleLeftCircleIcon,
  'triangle-left': TriangleLeftIcon,
  'triangle-right-circle-filled': TriangleRightCircleFilledIcon,
  'triangle-right-circle': TriangleRightCircleIcon,
  'triangle-right': TriangleRightIcon,
  'triangle-up-circle-filled': TriangleUpCircleFilledIcon,
  'triangle-up-circle': TriangleUpCircleIcon,
  'triangle-up': TriangleUpIcon,
  'triangle-updown': TriangleUpdownIcon,
  trophy: TrophyIcon,
  tune: TuneIcon,
  twitter: TwitterIcon,
  typography: TypographyIcon,
  underline: UnderlineIcon,
  untag: UntagIcon,
  upload: UploadIcon,
  username: UsernameIcon,
  variable: VariableIcon,
  videocam: VideocamIcon,
  'videocam-off': VideocamOffIcon,
  view: ViewIcon,
  'view-off': ViewOffIcon,
  'volume-down': VolumeDownIcon,
  'volume-off-filled': VolumeOffFilledIcon,
  'volume-off': VolumeOffIcon,
  'volume-up-filled': VolumeUpFilledIcon,
  'volume-up': VolumeUpIcon,
  wallet: WalletIcon,
  water: WaterIcon,
  'weather-cloud': WeatherCloudIcon,
  'weather-cloudy': WeatherCloudyIcon,
  'weather-rain': WeatherRainIcon,
  'weather-snow': WeatherSnowIcon,
  'weather-sun': WeatherSunIcon,
  'weather-thunder': WeatherThunderIcon,
  webhook: WebhookIcon,
  wifi: WifiIcon,
  'wifi-off': WifiOffIcon,
  'windows-close': WindowsCloseIcon,
  windows: WindowsIcon,
  'windows-maximize': WindowsMaximizeIcon,
  'windows-minimize': WindowsMinimizeIcon,
  'windows-restore': WindowsRestoreIcon,
  wing: WingIcon,
  youtube: YoutubeIcon,
  'zoom-in': ZoomInIcon,
  'zoom-out': ZoomOutIcon,
}

export type IconName = keyof typeof icons

export {
  AllIcon,
  AndroidIcon,
  ApiIcon,
  AppCheckIcon,
  AppPlusIcon,
  AppPushIcon,
  AppleIcon,
  AppsAddIcon,
  AppsFilledIcon,
  AppsIcon,
  ArchiveIcon,
  ArrowDownIcon,
  ArrowHookLeftDownIcon,
  ArrowHookLeftUpIcon,
  ArrowHookRightDownIcon,
  ArrowHookRightUpIcon,
  ArrowLeftCircleFilledIcon,
  ArrowLeftIcon,
  ArrowLeftUpIcon,
  ArrowRightIcon,
  ArrowRightUpIcon,
  ArrowRightUpSmallIcon,
  ArrowTurnLeftDownIcon,
  ArrowTurnLeftUpIcon,
  ArrowTurnRightDownIcon,
  ArrowTurnRightUpIcon,
  ArrowUpIcon,
  AssigneeIcon,
  AtomIcon,
  BadgeIcon,
  BankIcon,
  BaseballIcon,
  BasketballIcon,
  BearFilledIcon,
  BearIcon,
  BlockIcon,
  BoldIcon,
  BookCoverIcon,
  BookIcon,
  BookmarkFilledIcon,
  BookmarkIcon,
  BooleanIcon,
  BotFilledIcon,
  BotIcon,
  BreaktimeIcon,
  BriefcaseIcon,
  BrowserChromeIcon,
  BrowserEdgeIcon,
  BrowserFirefoxIcon,
  BrowserIeIcon,
  BrowserSafariIcon,
  BulletIcon,
  BusIcon,
  BusinessGuyIcon,
  ButtonRouteIcon,
  CalendarIcon,
  CallIcon,
  CallMissedIcon,
  CallOffIcon,
  CallOutgoingIcon,
  CallPullIcon,
  CameraIcon,
  CancelCircleFilledIcon,
  CancelCircleIcon,
  CancelIcon,
  CancelSmallIcon,
  CarIcon,
  CartAbandonedIcon,
  CartIcon,
  ChainReactionIcon,
  ChannelBtnFilledIcon,
  ChannelBtnIcon,
  ChannelBtnSmileFilledIcon,
  ChannelBtnSmileIcon,
  ChannelFilledIcon,
  ChannelIcon,
  ChannelLeftFilledIcon,
  ChannelLeftIcon,
  ChannelSmileFilledIcon,
  ChannelSmileIcon,
  ChannelUpFilledIcon,
  ChannelUpIcon,
  ChannelsListIcon,
  ChatBubbleFilledIcon,
  ChatBubbleIcon,
  ChatCancelIcon,
  ChatCheckFilledIcon,
  ChatCheckIcon,
  ChatErrorFilledIcon,
  ChatErrorIcon,
  ChatForwardIcon,
  ChatInfoIcon,
  ChatLightningIcon,
  ChatProgressFilledIcon,
  ChatProgressIcon,
  ChatPushIcon,
  ChatQuestionFilledIcon,
  ChatQuestionIcon,
  ChatReadyIcon,
  CheckAllIcon,
  CheckBoldIcon,
  CheckCircleFilledIcon,
  CheckCircleIcon,
  CheckIcon,
  CheckboxFilledIcon,
  CheckboxIcon,
  ChevronDownDoubleIcon,
  ChevronDownIcon,
  ChevronLeftDoubleIcon,
  ChevronLeftIcon,
  ChevronRightDoubleIcon,
  ChevronRightIcon,
  ChevronSmallDownIcon,
  ChevronSmallLeftIcon,
  ChevronSmallRightIcon,
  ChevronSmallUpIcon,
  ChevronSmallUpdownIcon,
  ChevronUpDoubleIcon,
  ChevronUpIcon,
  ChevronUpdownIcon,
  ClipIcon,
  ClockIcon,
  CloudDownloadIcon,
  CloudUploadIcon,
  CodeIcon,
  CoinIcon,
  CoinRecurringIcon,
  ColumnIcon,
  CommentFilledIcon,
  CommentIcon,
  CommentInIcon,
  CommentOutIcon,
  CommunicationIcon,
  ContactIcon,
  ContractIcon,
  CookieIcon,
  CouponIcon,
  CreditIcon,
  CreditcardIcon,
  DataIcon,
  DesktopIcon,
  DevicesIcon,
  DialogDownIcon,
  DialogUpIcon,
  DotIcon,
  DownloadIcon,
  DragableIcon,
  DragableSmallIcon,
  DropdownIcon,
  EditIcon,
  EmailFilledIcon,
  EmailIcon,
  EmailUnreadIcon,
  EmailUnsubscribedIcon,
  ErrorFilledIcon,
  ErrorIcon,
  ErrorTriangleFilledIcon,
  EventIcon,
  ExpandIcon,
  FaceAngryIcon,
  FaceGrinningIcon,
  FaceNomouthIcon,
  FaceSadIcon,
  FaceSmileAddIcon,
  FaceSmileFilledIcon,
  FaceSmileIcon,
  FaceWowIcon,
  FacebookIcon,
  FastForwardIcon,
  FastRewindIcon,
  FireIcon,
  FlagFilledIcon,
  FlashlightIcon,
  FlushIcon,
  FolderIcon,
  FullscreenExitIcon,
  FullscreenIcon,
  FunnelIcon,
  GhostFilledIcon,
  GhostIcon,
  GifIcon,
  GiftIcon,
  GlobeIcon,
  GoalIcon,
  GoogleIcon,
  GraphIcon,
  GroupFilledIcon,
  GroupIcon,
  HandIcon,
  HandLoveIcon,
  HandPointIcon,
  HandPushIcon,
  HandThumbdownFilledIcon,
  HandThumbdownIcon,
  HandThumbupFilledIcon,
  HandThumbupIcon,
  HandWaveIcon,
  HashtagIcon,
  HashtagLargeIcon,
  HeadingIcon,
  HeadsetIcon,
  HeadsetOffIcon,
  HearIcon,
  HeartFilledIcon,
  HeartIcon,
  HelpFilledIcon,
  HelpIcon,
  HexahedronIcon,
  HistoryIcon,
  HomeFilledIcon,
  HomeIcon,
  Hourglass1Icon,
  Hourglass2Icon,
  Hourglass3Icon,
  HyphenBoldIcon,
  HyphenIcon,
  ImageFailedIcon,
  ImageIcon,
  InIcon,
  InboxAllIcon,
  InboxIcon,
  InfoFilledIcon,
  InfoIcon,
  InstagramIcon,
  IntoPageIcon,
  InvertIcon,
  IosIcon,
  ItalicIcon,
  KakaoFilledIcon,
  KakaoIcon,
  KeyIcon,
  KeyboardIcon,
  LabIcon,
  LaptopIcon,
  LightbulbIcon,
  LightningFilledIcon,
  LightningIcon,
  LinkCopyIcon,
  LinkIcon,
  LinkOffIcon,
  LinkedinIcon,
  ListIcon,
  LockIcon,
  LockOpenIcon,
  MapPinIcon,
  MarkerPenIcon,
  MeIcon,
  MegaphoneFilledIcon,
  MegaphoneIcon,
  MenuFoldIcon,
  MenuIcon,
  MenuUnfoldIcon,
  MessagingAppIcon,
  MetroIcon,
  MicrophoneIcon,
  MicrophoneOffIcon,
  MinusCircleFilledIcon,
  MinusCircleIcon,
  MobileIcon,
  MobileMessagingIcon,
  MoonFilledIcon,
  MoonIcon,
  MoreIcon,
  MoreVerticalIcon,
  MouseIcon,
  MultiNodeIcon,
  MusicIcon,
  NaverIcon,
  NewIcon,
  NoteIcon,
  NoteLogoIcon,
  NotificationIcon,
  NotificationImportantIcon,
  NotificationOffIcon,
  NumberIcon,
  OfficeIcon,
  OpenInNewIcon,
  OrderIcon,
  OutIcon,
  PageDownloadIcon,
  PageIcon,
  PageZipIcon,
  PauseIcon,
  PencilIcon,
  PeopleIcon,
  PeopleListIcon,
  PersonAddIcon,
  PersonBlockedIcon,
  PersonCheckIcon,
  PersonCircleFilledIcon,
  PersonCircleIcon,
  PersonFilledIcon,
  PersonIcon,
  PersonRemoveIcon,
  PieChartIcon,
  PinIcon,
  PinOffIcon,
  PlaneIcon,
  PlayIcon,
  PlusCircleFilledIcon,
  PlusCircleIcon,
  PlusIcon,
  PlusSmallIcon,
  PointIcon,
  PowerIcon,
  PrintIcon,
  ProfileIcon,
  QrCodeIcon,
  RadioFilledIcon,
  RealtimeIcon,
  ReceiptIcon,
  RecipeIcon,
  RefreshCircleFilledIcon,
  RefreshIcon,
  RepeatIcon,
  RotateIcon,
  RowIcon,
  RulerIcon,
  SearchIcon,
  SecurityIcon,
  SecurityPersonIcon,
  SendFilledIcon,
  SendForwardFailedFilledIcon,
  SendForwardFailedIcon,
  SendForwardFilledIcon,
  SendForwardIcon,
  SendIcon,
  SettingsIcon,
  ShapeIcon,
  ShineIcon,
  ShoppingIcon,
  ShuffleIcon,
  SingleNodeIcon,
  SirenIcon,
  SmsFilledIcon,
  SmsIcon,
  SmsUnsubscribedIcon,
  SnoozeIcon,
  SoccerballIcon,
  SortingIcon,
  SpaceHorizontalIcon,
  SpaceVerticalIcon,
  SplitLeftIcon,
  SplitRightIcon,
  SquareIcon,
  SquaresFilledIcon,
  SquaresIcon,
  StarFilledIcon,
  StarIcon,
  StoreIcon,
  StrikethroughIcon,
  StringIcon,
  SurveyCheckIcon,
  SurveyIcon,
  SyncIcon,
  TabletIcon,
  TagIcon,
  TargetIcon,
  TemplateIcon,
  TextfieldIcon,
  TimeElapsedIcon,
  ToolIcon,
  TransferDisabledIcon,
  TransferIcon,
  TranslateIcon,
  TrashIcon,
  TrendingDownIcon,
  TrendingLeftCircleFilledIcon,
  TrendingLeftIcon,
  TrendingRightIcon,
  TrendingUpIcon,
  TriangleDownCircleFilledIcon,
  TriangleDownCircleIcon,
  TriangleDownIcon,
  TriangleLeftCircleFilledIcon,
  TriangleLeftCircleIcon,
  TriangleLeftIcon,
  TriangleRightCircleFilledIcon,
  TriangleRightCircleIcon,
  TriangleRightIcon,
  TriangleUpCircleFilledIcon,
  TriangleUpCircleIcon,
  TriangleUpIcon,
  TriangleUpdownIcon,
  TrophyIcon,
  TuneIcon,
  TwitterIcon,
  TypographyIcon,
  UnderlineIcon,
  UntagIcon,
  UploadIcon,
  UsernameIcon,
  VariableIcon,
  VideocamIcon,
  VideocamOffIcon,
  ViewIcon,
  ViewOffIcon,
  VolumeDownIcon,
  VolumeOffFilledIcon,
  VolumeOffIcon,
  VolumeUpFilledIcon,
  VolumeUpIcon,
  WalletIcon,
  WaterIcon,
  WeatherCloudIcon,
  WeatherCloudyIcon,
  WeatherRainIcon,
  WeatherSnowIcon,
  WeatherSunIcon,
  WeatherThunderIcon,
  WebhookIcon,
  WifiIcon,
  WifiOffIcon,
  WindowsCloseIcon,
  WindowsIcon,
  WindowsMaximizeIcon,
  WindowsMinimizeIcon,
  WindowsRestoreIcon,
  WingIcon,
  YoutubeIcon,
  ZoomInIcon,
  ZoomOutIcon,
}

/* eslint-enable */
export default icons
