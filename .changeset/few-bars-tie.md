...
`Icon` uses beta semantic color tokens and string pixel size values (`"10"`, `"12"`, `"16"`, `"20"`, `"24"`, `"36"`, `"44"`) to align with `AvatarSize` and `EmojiSize`. It defaults to `size="24"` and `color="icon-neutral"`. Icons are decorative by default and become meaningful images when an accessible name is provided.
