      ...
      `${Object.keys(categorizedTokens).map(
        (category) =>
          ...
          `${processTokensWithHovered(categorizedTokens[category])
            .map((token) => {
              const valueObject = createValueObject(dictionary, token)
...
export const customCss: CustomFormat = {
  name: 'custom/css',
...
function createValueObject(
  dictionary: Parameters<Format['formatter']>[0]['dictionary'],
  ...
  const ref = (() => {
    ...
    // NOTE: make sure to use `token.original.value` because `token.value` is
    // already resolved at this point.
    dictionary.getReferences(token.original.value).forEach((ref) => {
      value = value?.toString().replace(ref.value, ref.name) as string
    })
