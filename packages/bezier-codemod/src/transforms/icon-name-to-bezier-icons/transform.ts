...
const getComponentsToMigrate =
  (sourceFile: SourceFile) => (component: string) =>
    sourceFile
...
const isStringLiteral = (expression: Expression): expression is StringLiteral =>
  expression.getKindName() === StringLiteral.name

const isBezierIconImport = (declaration: ImportDeclaration) =>
  declaration.getModuleSpecifier().getLiteralValue() ===
  '@channel.io/bezier-icons'

const isReactImport = (declaration: ImportDeclaration) =>
  declaration.getModuleSpecifier().getLiteralValue() === 'react'

const isBezierReactImport = (declaration: ImportDeclaration) =>
  declaration.getModuleSpecifier().getLiteralValue() ===
  '@channel.io/bezier-react'

const addIconImport = (sourceFile: SourceFile) => (icon: string) => {
  const statements = sourceFile.getStatementsWithComments()
  const bezierReactImport = statements.findIndex(
    (statement) =>
      Node.isImportDeclaration(statement) && isBezierReactImport(statement)
  )
  const reactImportIndex = statements.findIndex(
    (statement) =>
      Node.isImportDeclaration(statement) && isReactImport(statement)
  )
  const bezierIconImport = sourceFile
    .getImportDeclarations()
    .find(isBezierIconImport)
  const importIndex =
    bezierReactImport > -1 ? bezierReactImport + 1 : reactImportIndex + 1
  ...
  if (!bezierIconImport) {
    ...
    bezierIconImport.getNamedImports().every((v) => v.getName() !== icon)
  ) {
...
const changeIconNameToBezierIcon =
  (sourceFile: SourceFile) =>
  (jsxElement: JsxSelfClosingElement, attribute: string) => {
    const leftContentAttribute = jsxElement.getAttribute(attribute) as
      | JsxAttribute
      | undefined
    ...
    if (
      ...
      isIconName(expression.getLiteralValue())
    ) {
  ...
  transformationTargets.reduce((acc, [component, attributes]) => {
    ...
    const migratedComponents = components.reduce((_acc, cur) => {
      const isMigrated = attributes
        .map((attribute) =>
          changeIconNameToBezierIcon(sourceFile)(cur, attribute)
        )
        .some((v) => v)
        ? 1
        : 0
