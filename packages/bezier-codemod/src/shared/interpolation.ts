...
import { getImportDeclaration, hasNamedImport } from '../utils/import.js'

export const interpolationTransform = (
  sourceFile: SourceFile,
  fromToObj: Record<string, string>
) => {
  sourceFile
      ...
      for (const [from, to] of Object.entries(fromToObj)) {
        if (!node.wasForgotten() && node.getText().includes(`\${${from}}`)) {
          node.replaceWithText(
            node.getText().replaceAll(`\${${from}}`, to).replaceAll(';;', ';')
          )
        }

        const bezierReactImport = getImportDeclaration(
          sourceFile,
