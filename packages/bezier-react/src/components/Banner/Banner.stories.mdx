...
interface BannerProps {
  ...
  /**
   * Whether to display close button at the top right of the banner.
   *
   * The close button itself does not hides the banner when clicked.
   * Use `onDismiss` handler to control the visibility of the banner.
   */
  dismissible?: boolean

  /**
   * Handler to be executed when the close button is clicked.
   */
  onDismiss?: () => void
}
```
