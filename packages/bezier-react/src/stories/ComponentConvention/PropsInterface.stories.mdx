...
import { KeyValueListItem } from '~/src/components/KeyValueListItem'
import { Icon } from '~/src/components/Icon'
import { ListItem, ListItemSize } from '~/src/components/ListItem'
import { OutlineItem } from '~/src/components/OutlineItem'
...
`content` prop은 일반적으로 `ReactNode` 타입을 가지지만, 컴포넌트에 따라 `string`, `BezierIcon` 타입을 강제하는 등 컴포넌트에 필요한 제한조건을 가할 수 있습니다.

<Canvas>
  <Story name="ContentProps:SectionLabelProps.content">
    <SectionLabel
      leftContent={{ icon: EmailFilledIcon }}
      content="수신함"
    />
  </Story>
...
가장 많이 사용되는 usecase로는 `leftContent` 또는 `rightContent` 타입을 `BezierIcon`으로 지정하고, 이것을 `Icon` 컴포넌트로 렌더링하는 것입니다.
...
<Canvas>
  <Story name="SideContentProps:ButtonProps.leftContent">
    <Button text="검색" leftContent={SearchIcon} />
  </Story>
  <Story name="SideContentProps:ButtonProps.rightContent">
    <Button text="가이드 확인하기" rightContent={ArrowRightUpIcon} styleVariant={ButtonStyleVariant.Tertiary} />
  </Story>
  <Story name="SideContentProps:SelectProps.leftContent">
    <Select leftContent={BookmarkFilledIcon} placeholder="작품 선택" />
  </Story>
  <Story name="SideContentProps:SelectProps.rightContent">
    <div style={{ width: '150px' }}>
      <SectionLabel
        leftContent={{ icon: PersonIcon }}
        content="김수한무거북이와두루미"
...
<Canvas>
  <Story name="ActivatableProps:ButtonProps:inactive">
    <Button styleVariant={ButtonStyleVariant.Tertiary} leftContent={LinkIcon} />
  </Story>
  <Story name="ActivatableProps:ButtonProps:active">
    <Button active styleVariant={ButtonStyleVariant.Tertiary} leftContent={LinkIcon} />
  </Story>
</Canvas>
...
<Canvas>
  <Story name="FormComponentProps:SelectProps.idle">
    <Select placeholder="태그를 선택하세요" leftContent={TagIcon} />
  </Story>
  <Story name="FormComponentProps:SelectProps.disabled">
    <Select placeholder="태그를 선택하세요" leftContent={TagIcon} disabled />
  </Story>
  <Story name="FormComponentProps:SelectProps.readOnly">
    <Select placeholder="태그를 선택하세요" leftContent={TagIcon} readOnly />
  </Story>
</Canvas>
